...
import {
  ArrowRight, Bell, Check, CheckCircle2, ChevronDown, CircleDot,
  ...
  Send, Settings, ShieldCheck, Sparkles, Sun, Sunrise, Users, X,
} from "lucide-react";
...
export function LumethaWorkspace({ userName, companyName, canReview = false }: { userName: string; companyId?: string; companyName?: string; companyDomain?: string | null; canReview?: boolean }) {
    ...
    <div className="flex min-h-screen">
      <aside className="hidden w-64 shrink-0 border-r border-[#e1e6e1] bg-[#fbfcfa] lg:flex lg:flex-col">
        <div className="flex h-[72px] items-center gap-3 border-b border-[#e7ebe7] px-5">
          <div className="grid h-9 w-9 place-items-center rounded-xl bg-[#183c2d] text-white"><Sunrise className="h-5 w-5" /></div>
        ...
        <nav className="flex-1 p-3">
          ...
          <SideButton icon={ClipboardCheck} label="Review queue" count={items.filter((i) => i.status === "pending_senior_review").length} />
          <p className="px-3 pb-2 pt-8 text-[10px] font-semibold uppercase tracking-[.14em] text-[#98a09b]">Workspace</p>
          <SideButton icon={Users} label="Delivery pod" />
      ...
      <section className="min-w-0 flex-1">
        <header className="flex h-[72px] items-center border-b border-[#e1e6e1] bg-white/90 px-4 backdrop-blur sm:px-7">
          <button className="mr-3 grid h-9 w-9 place-items-center rounded-lg border border-[#e1e6e1] lg:hidden" aria-label="Open navigation"><Menu className="h-4 w-4" /></button>
          ...
          <div className="ml-auto flex items-center gap-2"><button aria-label="Notifications" className="relative grid h-10 w-10 place-items-center rounded-xl text-[#647068] hover:bg-[#f1f4f1]"><Bell className="h-[18px] w-[18px]" /><span className="absolute right-2 top-2 h-2 w-2 rounded-full border-2 border-white bg-amber-500" /></button><button onClick={() => setComposer(true)} className="ml-1 inline-flex h-10 items-center gap-2 rounded-xl bg-[#183c2d] px-4 text-sm font-medium text-white shadow-sm hover:bg-[#102e22]"><Plus className="h-4 w-4" /><span className="hidden sm:inline">New dusk brief</span></button></div>
        </header>

        <div className="mx-auto max-w-[1680px] p-4 sm:p-7">
          <div className="overflow-hidden rounded-2xl border border-[#dfe6e0] bg-[linear-gradient(120deg,#173b2d,#24523e)] px-5 py-5 text-white shadow-[0_12px_30px_rgba(22,59,44,.12)] sm:px-7">
            <div className="flex flex-wrap items-center justify-between gap-5"><div><div className="flex items-center gap-2 text-xs font-medium text-emerald-200"><CloudMoon className="h-4 w-4" />ACTIVE DELIVERY CYCLE <span className="h-1 w-1 rounded-full bg-emerald-300" /> JUL 20 → JUL 21</div><h1 className="mt-2 text-2xl font-semibold tracking-[-.02em] sm:text-[28px]">Good evening, {userName.split(" ")[0]}.</h1><p className="mt-1 text-sm text-emerald-50/70">Drop the brief tonight. Wake up to reviewed, production-ready work.</p></div><div className="flex items-center gap-6 rounded-xl border border-white/10 bg-white/5 px-5 py-3"><div><p className="text-[10px] uppercase tracking-widest text-emerald-100/60">Dawn handoff</p><p className="mt-1 font-mono text-lg font-semibold">06:30 NPT</p></div><div className="h-9 w-px bg-white/15" /><div><p className="text-[10px] uppercase tracking-widest text-emerald-100/60">Cycle health</p><p className="mt-1 flex items-center gap-2 text-sm font-medium"><span className="h-2 w-2 rounded-full bg-emerald-300" />On track</p></div></div></div>
          </div>
          ...
          {view === "board" ? <div className="mt-5 grid gap-4 xl:grid-cols-4">{stages.map((stage, index) => {
            ...
            return <section key={stage.id} className="min-h-[490px] rounded-2xl border border-[#e0e5e0] bg-[#f0f3ef] p-3">
              ...
              {stage.id === "dusk_intake" && <button onClick={() => setComposer(true)} className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl border border-dashed border-[#bec9c1] py-3 text-xs font-medium text-[#627068] hover:border-[#6d927e] hover:bg-white"><Plus className="h-3.5 w-3.5" />Drop another brief</button>}
...
function IntakeModal({ onClose, onAdd }: { onClose: () => void; onAdd: (title: string, brief: string, priority: WorkItem["priority"], attachments: string[]) => void }) {
  ...
  return <div className="fixed inset-0 z-40 grid place-items-center bg-[#10251c]/40 p-4 backdrop-blur-[2px]" onMouseDown={onClose}><section role="dialog" aria-modal="true" aria-labelledby="intake-title" onMouseDown={(e) => e.stopPropagation()} className="w-full max-w-2xl rounded-2xl border border-white/50 bg-white p-6 shadow-2xl sm:p-7">
    ...
    <div className="mt-6 flex flex-wrap items-center justify-between gap-3 border-t border-[#edf0ed] pt-5"><select value={priority} onChange={(e) => setPriority(e.target.value as WorkItem["priority"])} className="h-10 rounded-xl border border-[#dbe1dc] bg-white px-3 text-xs"><option>Normal</option><option>High</option><option>Urgent</option></select><button disabled={!title.trim() || !brief.trim()} onClick={() => onAdd(title.trim(), brief.trim(), priority, files)} className="inline-flex h-10 items-center gap-2 rounded-xl bg-[#183c2d] px-4 text-sm font-medium text-white disabled:cursor-not-allowed disabled:opacity-40"><Send className="h-4 w-4" />Send to dusk intake</button></div>
  </section></div>;
}
...
function DetailDrawer({ item, canReview, onClose, onUpdate, onAdvance }: { item: WorkItem; canReview: boolean; onClose: () => void; onUpdate: (patch: Partial<WorkItem>) => void; onAdvance: () => void }) {
  ...
  return <div className="fixed inset-0 z-40 bg-[#10251c]/30" onMouseDown={onClose}><aside role="dialog" aria-modal="true" aria-labelledby="detail-title" onMouseDown={(e) => e.stopPropagation()} className="ml-auto h-full w-full max-w-3xl overflow-y-auto bg-[#fbfcfa] shadow-2xl">
    ...
    <div className="p-5 sm:p-8"><h2 id="detail-title" className="text-2xl font-semibold tracking-tight">{item.title}</h2><p className="mt-3 text-sm leading-6 text-[#657269]">{item.brief}</p>
      ...
      <div className="mt-6 flex justify-end">{item.status === "dusk_intake" && <button onClick={onAdvance} className="rounded-xl bg-[#183c2d] px-4 py-2.5 text-sm font-medium text-white">Accept & start work</button>}{item.status === "in_progress" && <button onClick={onAdvance} className="rounded-xl bg-[#183c2d] px-4 py-2.5 text-sm font-medium text-white">Submit for senior review</button>}{item.status === "pending_senior_review" && <button disabled={!canReview || !ready} onClick={() => onUpdate({ status: "dawn_shipped", review })} className="inline-flex items-center gap-2 rounded-xl bg-[#d99328] px-4 py-2.5 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-40"><Sun className="h-4 w-4" />Approve for dawn shipping</button>}</div>
