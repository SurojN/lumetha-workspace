...
export function AuthForm({ mode }: { mode: "login" | "register" }) {
  ...
  return <main className="grid min-h-screen place-items-center bg-slate-50 p-5">
    <form action={formAction} className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
      ...
      <button disabled={pending} className="mt-6 w-full rounded-lg bg-emerald-600 px-4 py-2.5 font-medium text-white disabled:opacity-60">{pending ? "Please wait…" : isRegister ? "Create access request" : "Sign in"}</button>
