...
export function AuthForm({ mode }: { mode: "login" | "register" }) {
  ...
  return <main className="grid min-h-screen place-items-center bg-slate-50 p-5">
    <form action={formAction} className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
      <div className="mb-7"><div className="mb-4 grid h-10 w-10 place-items-center rounded-xl bg-[#202b4b] font-bold text-white">L</div><h1 className="text-2xl font-semibold">{isRegister ? "Create your account" : "Welcome back"}</h1><p className="mt-1 text-sm text-slate-500">{isRegister ? "Start a secure workspace for your company." : "Sign in to your company workspace."}</p></div>
      ...
      <button disabled={pending} className="mt-6 w-full rounded-lg bg-[#202b4b] px-4 py-2.5 font-medium text-white disabled:opacity-60">{pending ? "Please wait…" : isRegister ? "Create account" : "Sign in"}</button>
