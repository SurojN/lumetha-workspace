...
export default async function AdminPage() {
  ...
  const [companies, projects, cycles, tasks, developers, audit] = await Promise.all([
  ...
  return <AdminCommandCenter userName={user.name ?? user.email ?? "Lumetha admin"} companies={companies} projects={projects} cycles={cycles.map((cycle) => ({ ...cycle, date: cycle.date.toISOString(), scopeLockedAt: cycle.scopeLockedAt?.toISOString() ?? null }))} tasks={tasks.map((task) => ({ ...task, updatedAt: task.updatedAt.toISOString() }))} developers={developers} audit={audit.map((event) => ({ ...event, createdAt: event.createdAt.toISOString() }))} />;
}
